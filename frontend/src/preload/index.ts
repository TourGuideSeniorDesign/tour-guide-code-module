...
const rosBridgeAPI = {
  connect: (url: string): Promise<void> => ipcRenderer.invoke("ros:connect", url),
  disconnect: (): Promise<void> => ipcRenderer.invoke("ros:disconnect"),
  getState: (): Promise<RosBridgeSnapshot> => ipcRenderer.invoke("ros:get-state"),
  publishTourControl: (
    message: TourControlMessage,
  ): Promise<PublishResult> =>
    ipcRenderer.invoke("ros:publish-tour-control", message),
  onState: (listener: (snapshot: RosBridgeSnapshot) => void): (() => void) => {
    const wrapped = (_event: Electron.IpcRendererEvent, snapshot: RosBridgeSnapshot) => {
      listener(snapshot);
    };
