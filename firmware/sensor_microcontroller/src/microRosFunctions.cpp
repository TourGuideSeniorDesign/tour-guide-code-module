...
#include <autogiro_interfaces/msg/fan_speed.h>
#include <autogiro_interfaces/msg/fingerprint.h>
#include <autogiro_interfaces/msg/lidar.h>
#include <autogiro_interfaces/msg/light.h>
...
autogiro_interfaces__msg__FanSpeed fanMsg;
autogiro_interfaces__msg__Light lightMsg;
autogiro_interfaces__msg__Lidar lidarMsg;
#ifdef ROS
autogiro_interfaces__msg__Sensors sensorMsg;
#elif ROS_DEBUG
autogiro_interfaces__msg__RefSpeed msg;
#endif
...
bool create_entities() {
  ...
  RCCHECK(rclc_publisher_init_default(
      &fingerprintPublisher, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(autogiro_interfaces, msg, Fingerprint),
      "fingerprint"));
  ...
  RCCHECK(rclc_subscription_init_default(
      &fanSubscriber, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(autogiro_interfaces, msg, FanSpeed),
      "fan_duty_cycles"));

  // Create subscriber
  RCCHECK(rclc_subscription_init_default(
      &lightSubscriber, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(autogiro_interfaces, msg, Light), "light"));

  // Create subscriber
  RCCHECK(rclc_subscription_init_default(
      &lidarSubscriber, &node,
      ROSIDL_GET_MSG_TYPE_SUPPORT(autogiro_interfaces, msg, Lidar), "lidar"));

  // create executor
  // Number of handles = # timers + # subscriptions + # clients + # services
  executor = rclc_executor_get_zero_initialized_executor();
  RCCHECK(rclc_executor_init(&executor, &support.context, 5, &allocator));
  RCCHECK(rclc_executor_add_subscription(&executor, &fanSubscriber, &fanMsg,
                                         &fan_subscription_callback,
                                         ON_NEW_DATA));
  RCCHECK(rclc_executor_add_subscription(&executor, &lightSubscriber, &lightMsg,
                                         &light_subscription_callback,
                                         ON_NEW_DATA));
  RCCHECK(rclc_executor_add_subscription(&executor, &lidarSubscriber, &lidarMsg,
                                         &lidar_subscription_callback,
                                         ON_NEW_DATA));
...
void publishFingerprint(uint8_t fingerprintID) {
  autogiro_interfaces__msg__Fingerprint fingerprintMsg;
  fingerprintMsg.user_id = fingerprintID;
  RCSOFTCHECK(rcl_publish(&fingerprintPublisher, &fingerprintMsg, NULL));
}

void publishError(const bool joystick_adc_error,
                  const bool ultrasonic_adc_error, const bool fingerprint_error,
                  const bool imu_error) {
  autogiro_interfaces__msg__SensorError sensorErrorMsg;
...
void fan_subscription_callback(const void *msgin) {
  const auto *msg = (const autogiro_interfaces__msg__FanSpeed *)msgin;
...
void light_subscription_callback(const void *msgin) {
  const auto *msg = (const autogiro_interfaces__msg__Light *)msgin;
  const int lightState = msg->state;
  setLight(lightState);
}

void lidar_subscription_callback(const void *msgin) {
  const auto *msg = (const autogiro_interfaces__msg__Lidar *)msgin;
  if (msg->state == 0) {
    lidarState(false);
  } else if (msg->state == 1) {
    lidarState(true);
...
void transmitMsg(RefDisplacement thetaRef, RefSpeed omegaRef,
                 USData ultrasonicData, PIRSensors pirSensors,
